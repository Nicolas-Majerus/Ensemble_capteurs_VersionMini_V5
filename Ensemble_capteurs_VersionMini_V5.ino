...
Serial.println(moy_pm10);
Serial.print("moyenne PM2.5= ");
Serial.println(moy_pm25);
Serial.println("#2");
...
//    strcat(commande, "\r");
    ...
    char * commandeSigfox = "";                         //Creation du message SigFox
    commandeSigfox = prepareForSigfox(dht.readTemperature(), dht.readHumidity(), moy_pm10, moy_pm25);   
    
Serial.println(commandeSigfox);                               //Affichage dans le moniteur du message envoyé
    sigfox.write(commandeSigfox);                             //Envoi au modem de la commande
Serial.println("#3");
    free(commandeSigfox);                                     //Nettoyage de la commande pour gain de mémoire
Serial.println("#4");
...
char * prepareForSigfox(int temperature, int humidity, int pm10, int pm25) 
{
  ...
  sprintf(dataString, "AT$SF=%02X%02X%02X%02X\r", pm10, pm25, humidity, temperature);
  return dataString;
}
